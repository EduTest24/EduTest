...
// https://script.google.com/macros/s/AKfycbxhCFw2pekV7h62EE_y4DC3MzyffPIPs1U4A9zjDvsvcYa-A8qfl9LemmnfV4Fb-UqkmA/exec
// https://script.google.com/macros/s/AKfycbxrD_IgE4d18isJJOj0Psowp919DbUg5h4QxQoai1NChcIdaM87cAF1WBnk_RFIJa3ZHQ/exec
// function validateEmail() {
//   var emailField = document.getElementById("email").value;

//   // Call the Google Apps Script web app to check for the email
//   fetch(
//     "https://script.google.com/macros/s/AKfycbxrD_IgE4d18isJJOj0Psowp919DbUg5h4QxQoai1NChcIdaM87cAF1WBnk_RFIJa3ZHQ/exec?email=" +
//       emailField
//   )
//     .then((response) => response.json())
//     .then((data) => {
//       if (data.success) {
//         // If the email exists, set the userName value with the full name
...
//         ).innerHTML = `Test Details : <i class="fas fa-user"></i> ${userName}`;
//       } else {
//         // If email doesn't exist, give options to the user
//         if (confirm("Email not found. Do you want to register?")) {
//           // Redirect to the registration page if the user clicks "OK"
//           window.location.href =
//             "https://script.google.com/macros/s/AKfycbyQupl2E87Gz_ZkcuntfUAq591QDmCHTXEOzy2kW4e24FbVRsdx7D3ufhI60qCRfenQog/exec";
//         } else {
...
// }
// Users Appscript
// https://script.google.com/macros/s/AKfycbybsiQ2Ds63i8kx0UBdpCU1LanfpTUzqj4jM9JSrIj12d4QGqWRKqtCzqbrE6K1vdIAww/exec
  ...
  fetch(
    ...
    .then((data) => {
      ...
      if (data.success) {
        ...
        // If email doesn't exist, notify the user
...
// Function to open the registration page in a new tab
...
let switchCount = 0; // Count how many times the user switches tabs
let timeOut = null; // To track when the user leaves the tab
const maxSwitches = 3; // Max number of allowed tab switches
let testSubmitted = false; // Flag to check if the test has been submitted
let trackingStarted = false; // Flag to check if tracking started

const modal = document.getElementById("switch-switchModal");
const modalMessage = document.getElementById("switch-modalMessage");
const closeModalButton = document.getElementById("switch-closeModal");
...
// Function to start tracking tab switches
...
// Function to stop tracking tab switches
...
function handleVisibilityChange() {
  if (testSubmitted) return; // Stop further actions after the test is submitted

  const warningMessage = document.getElementById("switch-cheatWarning");
  const switchCountElement = document.getElementById("switch-switches");
  const timeAwayElement = document.getElementById("switch-timeAway");

  if (document.hidden) {
    // When the user leaves the tab, start timing how long they're away
    timeOut = Date.now();
  } else {
    // When they return, calculate the time spent away
    let timeAwayNow = Math.floor((Date.now() - timeOut) / 1000);

    // Increment the switch count
    switchCount++;
    switchCountElement.textContent = switchCount;

    // Display modal with a warning message
    modalMessage.textContent = `You switched tabs for ${timeAwayNow} seconds. Switching tabs is not allowed!`;
    modal.style.display = "flex"; // Show the modal

    // Update the time away element
    timeAwayElement.textContent = `You were away for ${timeAwayNow} seconds.`;

    // Check if the user switched too many times
    if (switchCount >= maxSwitches) {
      modalMessage.textContent =
        "You've switched tabs too many times. Test is being submitted!";
      modal.style.display = "flex"; // Show modal before submission
      confirmSubmit();
    }
  }
}
...
function selectOption(questionIndex, optionIndex, type) {
  ...
  if (!question.interactionLog) {
    question.interactionLog = [];
  }
  if (question.changeCount === undefined) {
    question.changeCount = 0;
  }

  // Record interaction and increment change count if the option changes
  if (type === "SCQ" && question.userSelectedOption !== optionIndex) {
    question.changeCount++;
  } else if (
    type === "MCQ" &&
    (!question.userSelectedOption ||
  ...
  if (type === "SCQ") {
    ...
    const selectedIndex = question.userSelectedOption.indexOf(optionIndex);
    if (selectedIndex > -1) {
  ...
  displayQuestion(questionIndex); // Re-render the question UI
}
...
function recordNumericalAnswer(questionIndex, answer) {
  ...
  // Record interaction and increment change count if the answer changes
  if (question.userSelectedOption !== parseInt(answer)) {
    question.changeCount++;
  }

  question.interactionLog.push({
    time: (Date.now() - questionStartTime) / 1000,
    inputValue: parseInt(answer),
    interactionType: "numerical",
  });
...
function confirmSubmit() {
  ...
  testSubmitted = true; // Set the flag to stop tracking tab switches
  stopTracking(); // Stop the tab switch tracking
  clearInterval(timerInterval);
  showPerformanceAnalysis();
  displayUserBehaviorAnalysis();
...
function analyzeUserBehavior() {
  const analysisResults = questions.map((question, index) => {
    ...
    const changes = question.changeCount || 0;
    const timeTaken = question.timeTaken ? question.timeTaken.toFixed(2) : 0;

    let behaviorComment = "Stable";
    if (changes >= 3) {
      behaviorComment = "Indecisive";
    } else if (changes === 0) {
      behaviorComment = "Confident";
    }
...
function displayUserBehaviorAnalysis() {
  const analysisResults = analyzeUserBehavior();

  let html = `<div class="behavior-analysis">
...
<td>${result.behaviorComment}</td>
</tr>`;
  });

  html += `</table></div>`;

  // Inject the HTML into the div with the unique ID
...
//   function displayUserBehaviorAnalysis(page = 1) {
//     const analysisResults = analyzeUserBehavior();
...
//       controls += `<button onclick="changePage(${i})" class="${
...
//     currentPage = page;
//     displayUserBehaviorAnalysis(currentPage);
//   }
